...
import {
  chemicalTestingProducts,
  type ChemicalTestCategory,
  type ChemicalTestProduct,
} from "../data/chemicalTestingData";
...
// ✅ slug helper (same pattern as NDT & Physical)
const slugify = (text: string) =>
  text.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "");

const ChemicalTestingPage: React.FC = () => {
  ...
  const allProducts: FlatProduct[] =
    chemicalTestingProducts.flatMap(
      (category: ChemicalTestCategory) =>
        category.products.map(
          (product: ChemicalTestProduct) => ({
            id: product.id,
            name: product.name,
            image: product.image,
  ...
  const categories: string[] = [
    "All",
    ...chemicalTestingProducts.map(
      (c: ChemicalTestCategory) => c.categoryName
    ),
  ];
    ...
    <div className="min-h-screen bg-gray-50 px-4 py-8 sm:px-6">
      ...
      <div className="mb-8 text-center">
        ...
        <p className="mt-2 text-sm text-gray-600">
          Accurate chemical analysis & laboratory testing solutions
        </p>
      </div>
      ...
      <div className="mb-8 w-full">
        <div className="flex w-full flex-col overflow-hidden rounded-full border border-gray-300 bg-white shadow-sm sm:flex-row">
          ...
          <input
            type="text"
            placeholder="Search chemical testing equipment"
            value={search}
        ...
        {filteredProducts.length > 0 ? (
          filteredProducts.map((product: FlatProduct) => (
            <div
              ...
              onClick={() =>
                navigate(
                  `/products/chemical-testing/${slugify(
                    product.name
                  )}`
                )
              }
...
export default ChemicalTestingPage;

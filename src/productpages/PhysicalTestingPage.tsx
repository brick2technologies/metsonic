...
import {
  physicalTestingProducts,
  type PhysicalTestCategory,
  type PhysicalTestProduct,
} from "../data/physicalTestingData";
...
// ✅ slug helper (same pattern as NDT)
const slugify = (text: string) =>
  text.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "");

const PhysicalTestingPage: React.FC = () => {
  ...
  const allProducts: FlatProduct[] =
    physicalTestingProducts.flatMap(
      (category: PhysicalTestCategory) =>
        category.products.map(
          (product: PhysicalTestProduct) => ({
            id: product.id,
            name: product.name,
            image: product.image,
  ...
  const categories: string[] = [
    "All",
    ...physicalTestingProducts.map(
      (c: PhysicalTestCategory) => c.categoryName
    ),
  ];
    ...
    <div className="min-h-screen bg-gray-50 px-4 py-8 sm:px-6">
      ...
      <div className="mb-8 text-center">
        ...
        <p className="mt-2 text-sm text-gray-600">
          Mechanical & material testing solutions
        </p>
      </div>
      ...
      <div className="mb-8 w-full">
        <div className="flex w-full flex-col overflow-hidden rounded-full border border-gray-300 bg-white shadow-sm sm:flex-row">
          ...
          <input
            type="text"
            placeholder="Search physical testing equipment"
            value={search}
        ...
        {filteredProducts.length > 0 ? (
          filteredProducts.map((product: FlatProduct) => (
            <div
              ...
              onClick={() =>
                navigate(
                  `/products/physical-testing/${slugify(
                    product.name
                  )}`
                )
              }
...
export default PhysicalTestingPage;
